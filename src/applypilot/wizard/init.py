...
def _setup_profile() -> dict:
    ...
    console.print("[dim]Optional. Each line becomes a bullet on your resume (e.g. '1st rank in X competition').[/dim]")
    ach_raw = Prompt.ask("Achievements (comma-separated, or leave blank)", default="")
    profile["achievements"] = [a.strip() for a in ach_raw.split(",") if a.strip()]
