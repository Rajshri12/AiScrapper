...
def get_stats(conn: sqlite3.Connection | None = None) -> dict:
    ...
    # Notification stage
    stats["referral_notified"] = conn.execute(
        "SELECT COUNT(*) FROM jobs WHERE notify_type = 'referral'"
    ).fetchone()[0]

    stats["blocked_notified"] = conn.execute(
        "SELECT COUNT(*) FROM jobs WHERE notify_type = 'blocked'"
    ).fetchone()[0]

    stats["captcha_notified"] = conn.execute(
        "SELECT COUNT(*) FROM jobs WHERE notify_type = 'captcha'"
    ).fetchone()[0]
...
def get_jobs_by_stage(conn: sqlite3.Connection | None = None,
    ...
    conditions = {
        ...
        "pending_tailor": (
            ...
            "AND COALESCE(legitimacy_score, '1') != '3'"
        ),
        "tailored": "tailored_resume_path IS NOT NULL",
        "pending_apply": (
            "tailored_resume_path IS NOT NULL AND applied_at IS NULL "
            "AND application_url IS NOT NULL "
            "AND COALESCE(notify_type, '') != 'referral'"
        ),
