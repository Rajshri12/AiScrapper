...
def run_job(job: dict, port: int, worker_id: int = 0,
    ...
    # On Windows, claude is installed as claude.cmd via npm and subprocess
    # won't find it without the .cmd extension (unlike PowerShell/cmd.exe).
    import shutil as _shutil
    _claude_exe = _shutil.which("claude") or _shutil.which("claude.cmd") or "claude"
    cmd = [
        _claude_exe,
        "--model", model,
        "-p",
