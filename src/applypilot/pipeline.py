...
STAGE_ORDER = ("discover", "enrich", "score", "tailor", "cover", "pdf", "notify")
...
STAGE_META: dict[str, dict] = {
    ...
    "notify":   {"desc": "Email notifications (referral targets + blocked jobs)"},
}
...
def _run_discover(workers: int = 1) -> dict:
    """Stage: Job discovery — JobSpy, Workday, ATS APIs, remote boards, India, smart-extract."""
    stats: dict = {"jobspy": None, "workday": None, "ats": None,
                   "websearch": None, "india": None, "smartextract": None}
    ...
    # ATS API providers (Greenhouse, Ashby, Lever, Workable, SmartRecruiters)
    console.print("  [cyan]ATS API providers (Greenhouse/Ashby/Lever/Workable)...[/cyan]")
    try:
        from applypilot.discovery.ats import run_ats_discovery
        r = run_ats_discovery()
        stats["ats"] = f"ok ({r['new']} new)"
    except Exception as e:
        log.error("ATS discovery failed: %s", e)
        console.print(f"  [red]ATS error:[/red] {e}")
        stats["ats"] = f"error: {e}"

    # Remote boards (RemoteOK, Remotive, Himalayas, WeWorkRemotely)
    console.print("  [cyan]Remote boards (RemoteOK/Remotive/Himalayas/WWR)...[/cyan]")
    try:
        from applypilot.discovery.websearch import run_websearch_discovery
        r = run_websearch_discovery()
        stats["websearch"] = f"ok ({r['new']} new)"
    except Exception as e:
        log.error("WebSearch discovery failed: %s", e)
        console.print(f"  [red]WebSearch error:[/red] {e}")
        stats["websearch"] = f"error: {e}"

    # India boards (Naukri, Foundit)
    console.print("  [cyan]India boards (Naukri/Foundit)...[/cyan]")
    try:
        from applypilot.discovery.india import run_india_discovery
        r = run_india_discovery()
        stats["india"] = f"ok ({r['new']} new)"
    except Exception as e:
        log.error("India discovery failed: %s", e)
        console.print(f"  [red]India error:[/red] {e}")
        stats["india"] = f"error: {e}"
...
def _run_tailor(min_score: int = 6, validation_mode: str = "normal") -> dict:
...
def _run_cover(min_score: int = 6, validation_mode: str = "normal") -> dict:
...
def _run_notify(min_score: int = 6, referral_threshold: int = 9) -> dict:
    """Stage: Email notifications — referral targets and blocked/manual ATS jobs."""
    try:
        from applypilot.scoring.notify import run_notifications
        result = run_notifications(min_score=min_score,
                                   referral_threshold=referral_threshold)
        sent = result["referral_sent"] + result["blocked_sent"]
        return {"status": "ok", **result,
                "summary": (
                    f"{result['referral_sent']} referral, "
                    f"{result['blocked_sent']} blocked, "
                    f"{result['captcha_sent']} captcha"
                )}
    except Exception as e:
        log.error("Notify stage failed: %s", e)
        return {"status": f"error: {e}"}
...
def _run_stage_streaming(
    ...
    if stage == "notify":
        kwargs["min_score"] = min_score
        kwargs["referral_threshold"] = referral_threshold
    ...
    for name in ordered:
        ...
        t = threading.Thread(
            ...
            args=(name, tracker, stop_event, min_score, workers,
                  validation_mode, referral_threshold),
            name=f"stage-{name}",
            daemon=True,
        )
        threads[name] = t
